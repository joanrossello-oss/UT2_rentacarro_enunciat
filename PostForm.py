from flask_wtf import FlaskForm
from wtforms import StringField, SubmitField, DateField, SelectField, validators, ValidationError, IntegerField
from wtforms.validators import DataRequired, Length, NumberRange
from datetime import date
from flask import session

class PostForm(FlaskForm):
    # mètode per comprovar si la fetxa es posterior a la fetxa actual
    def comprovar_fetxa(form, fetxa):
        if fetxa.data < date.today(): # date.today() -> obtenir la fetxa actual
            raise ValidationError('La fetxa no pot ser anterior a la actual.')
        
    def comprovar_carro_existent(form, camp):
        iniciReserva = form.iniciReserva.data
        finalReserva = form.finalReserva.data
        carro = form.carro.data

        reserves = session.get('reserves',[])

        for r in reserves:
            # comprar les dades de les reserves existents amb la peticio de post a fer
            if r['iniciReserva'] <= iniciReserva and r['finalReserva'] >= finalReserva and r['carro'] == carro:
                raise ValidationError('No pots reservar el carro per el dia seleccionat, ja es troba reservat.')
    
    nom = StringField('nom', validators=[DataRequired(), Length(max=128)])
    llinatges = StringField('llinatges', validators=[DataRequired(), Length(max=128)])
    iniciReserva = DateField('iniciReserva', format='%Y-%m-%d', default=date.today, validators=[DataRequired(), comprovar_fetxa])
    finalReserva = DateField('finalReserva', format='%Y-%m-%d', default=date.today, validators=[DataRequired(), comprovar_fetxa])
    idcarro = SelectField('idcarro', choices=['Carro simple', 'carro 2', 'carro 3'], validators=[DataRequired(), comprovar_carro_existent])
    horareserva = IntegerField('horareserva', validators=[DataRequired(), NumberRange(min=1, max=23)])
    horaretorno = IntegerField('horaretorno', validators=[DataRequired(), NumberRange(min=1, max=23)])
    submit = SubmitField('Enviar')

    
